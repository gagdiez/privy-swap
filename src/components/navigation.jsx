...
export const Navigation = () => {
  ...
  const { walletId, getBalance } = useNEAR();

  useEffect(() => {
    if (authenticated) {
      const userId = user.google ? user.google.email : user.email ? user.email.address : user.github.username
      ...
      <nav className="navbar navbar-expand-lg">
        <div className="container-fluid">
          ...
          <div className="navbar-nav pt-1">
            {authenticated ? <span className="badge text-dark small"> <br />{walletId}: {balance} Ⓝ</span> : null}
